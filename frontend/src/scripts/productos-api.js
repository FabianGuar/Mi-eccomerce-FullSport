...
console.log('🛍 Inicializando productos-api.js');
...
async function addToCartFromAPI(productId) {
    ...
    try {
        ...
        const producto = allProductsFromAPI.find(p => (p._id || p.id) === productId);
        ...
        if (producto.quantity === 0) {
            showNotification('⚠️ Producto agotado', 'warning');
            return;
        }
        
        // USAR LA MISMA KEY QUE carrito.js
        ...
        if (existingItem) {
            ...
            if (existingItem.quantity < producto.quantity) {
                existingItem.quantity += 1;
                showNotification(`✅ Cantidad actualizada: ${existingItem.quantity}`, 'success');
            } else {
            ...
            cart.push({
                ...
                quantity: 1,
                maxStock: producto.quantity,
                addedAt: new Date().toISOString() // Añadir fecha
            });
            showNotification('✅ Producto agregado al carrito', 'success');
        }
        
        // Guardar carrito actualizado con la MISMA KEY
        ...
        // Disparar evento para que carrito.js se actualice (asíncrono)
        setTimeout(() => {
            window.dispatchEvent(new CustomEvent('cartUpdated', { 
                detail: { items: cart, total: cart.reduce((sum, item) => sum + (item.price * item.quantity), 0) } 
            }));
        }, 0);
...
function updateCartCount() {
    ...
    const tryUpdate = () => {
        ...
        console.log('🔢 Actualizando contador. Items:', totalItems);
        ...
        if (cartCountElement) {
            ...
            console.warn('⚠️ Elemento #cart-counter no encontrado, reintentando...');
            return false;
        }
    };
    ...
    if (!tryUpdate()) {
        // Si falla, reintentar después de 100ms
        setTimeout(() => {
            if (!tryUpdate()) {
                // Último intento después de 500ms
...
function initializeProductsAPI() {
    console.log('🎬 Inicializando listeners de carrito...');
    
    // Actualizar contador inicial
    updateCartCount();
    
    // Escuchar cambios en el carrito desde cualquier parte
    window.addEventListener('cartUpdated', (e) => {
        console.log('🔄 Carrito actualizado, refrescando contador...', e.detail);
        updateCartCount();
    });
    
    // Escuchar cambios de storage (sincronización entre pestañas)
    window.addEventListener('storage', (e) => {
        if (e.key === 'ecommerce-cart-data') {
    ...
    console.log('✅ Listeners de carrito configurados');
}

// Ejecutar cuando el DOM esté listo
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeProductsAPI);
} else {
    initializeProductsAPI();
}
